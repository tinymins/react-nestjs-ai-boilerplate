...
import { workspaceService, toWorkspaceOutput } from "./workspace.service";
...
export class WorkspaceRouter {
	...
	async create(input: z.infer<typeof CreateWorkspaceInputSchema>, @Ctx() ctx: Context) {
	...
	async update(input: z.infer<typeof UpdateWorkspaceInputSchema>, @Ctx() ctx: Context) {
		const updated = await workspaceService.update(input.id, input, ctx.userId!, ctx.language);
		return toWorkspaceOutput(updated);
	}

	@UseMiddlewares(requireUser)
	@Mutation({ input: DeleteWorkspaceInputSchema, output: z.object({ id: z.string() }) })
	async delete(input: z.infer<typeof DeleteWorkspaceInputSchema>, @Ctx() ctx: Context) {
